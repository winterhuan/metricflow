...
class SparkSqlExpressionRenderer(DefaultSqlExpressionRenderer):
    ...
    def visit_percentile_expr(self, node: SqlPercentileExpression) -> SqlExpressionRenderResult:
        ...
        if node.percentile_args.function_type is SqlPercentileFunctionType.CONTINUOUS:
            # Spark uses PERCENTILE for continuous percentile
            return SqlExpressionRenderResult(
                sql=f"PERCENTILE({arg_rendered.sql}, {percentile})",
                execution_parameters=params,
            )
        elif node.percentile_args.function_type is SqlPercentileFunctionType.DISCRETE:
            # Spark uses PERCENTILE_DISC for discrete percentile
            return SqlExpressionRenderResult(
                sql=f"PERCENTILE_DISC({percentile}) WITHIN GROUP (ORDER BY {arg_rendered.sql})",
                execution_parameters=params,
            )
        elif node.percentile_args.function_type is SqlPercentileFunctionType.APPROXIMATE_CONTINUOUS:
            # Spark uses APPROX_PERCENTILE for approximate continuous percentile
            return SqlExpressionRenderResult(
            ...
            raise RuntimeError(
                "Approximate discrete percentile aggregate not supported for Spark. Set "
